...
SELECT 
  ...
  FORMAT_DATE('%G-%V', DATE(tpep_pickup_datetime)) AS year_week,
  EXTRACT(DAYOFWEEK FROM tpep_pickup_datetime) AS weekday,
  COUNT(*) AS total_trips
FROM `sales-etl-481918.transformed_data.cleaned_and_filtered`
GROUP BY trip_date, year, month, year_week, weekday;
...
CREATE OR REPLACE VIEW `views_fordashboard.trips_by_hour_zone` AS
SELECT 
  EXTRACT(HOUR FROM t.tpep_pickup_datetime) AS pickup_hour,
  z.Borough,
  z.Zone,
  COUNT(*) AS total_trips
FROM `sales-etl-481918.transformed_data.cleaned_and_filtered` t
JOIN `sales-etl-481918.raw_yellowtrips.tax_zone` z
  ON t.PULocationID = z.LocationID
GROUP BY pickup_hour, z.Borough, z.Zone;


SELECT * FROM `views_fordashboard.trips_by_hour_zone` LIMIT 1000;

SELECT DISTINCT Zone FROM `views_fordashboard.trips_by_hour_zone`;
...
SELECT 
    ...
    COUNT(*) AS total_trips
FROM `sales-etl-481918.transformed_data.cleaned_and_filtered` t
JOIN `sales-etl-481918.raw_yellowtrips.tax_zone` pz 
    ON t.PULocationID = pz.LocationID
JOIN `sales-etl-481918.raw_yellowtrips.tax_zone` dz 
